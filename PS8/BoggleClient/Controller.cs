        ...
        private async void SetUp()
        {
            ...
            try
            {
                ...
                using (HttpClient client = CreateClient())
                {
                    ...
                    {
                        String result = await response.Content.ReadAsStringAsync();
                        dynamic items = JsonConvert.DeserializeObject(result).ToString();
                        JObject obj = JObject.Parse(items); // it will parse deserialize Json object
            ...
            try
            {
                ...
                using (HttpClient client = CreateClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                       ...
                       // dynamic items = JsonConvert.DeserializeObject(result).ToString();
                        //JObject obj = JObject.Parse(items); // it will parse deserialize Json object
                        //var wordScore = obj["Score"]; // now after parsing deserialize Json object you can get individual values by key i.e.
                        view.newWord(word);
                    }
                    else
                    {
